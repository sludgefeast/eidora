...
import org.eidora.worker.NotificationHelper
...
class PhotoSyncWorker(
    ...
    private suspend fun doFullSync(): Result {
        try { setForeground(NotificationHelper.syncForegroundInfo(applicationContext, 0, applicationContext.getString(org.eidora.R.string.notif_scanning_start))) } catch (t: Throwable) { android.util.Log.w("FACES", "setForeground failed", t) }

        val folderWhitelist = try {
            org.eidora.data.settings.SettingsProvider.get(applicationContext).getFolderWhitelist()
        } catch (t: Throwable) {
                ...
                try {
                    setForegroundAsync(
                        NotificationHelper.syncForegroundInfo(
                            applicationContext, 0, applicationContext.getString(org.eidora.R.string.notif_scanning, count)
                        )
                    )
        ...
        setProgress(workDataOf(KEY_PROGRESS to 100, KEY_STATUS to applicationContext.getString(org.eidora.R.string.notif_done)))
        ...
        if (xmpRegions.isNotEmpty()) {
            xmpRegions.forEach { xmpRegion ->
                try {
                    ...
                    val qualityScore = org.eidora.util.FaceQuality.computeFast(
    ...
    private suspend fun runMlKit(file: File, photoId: String) {
        ...
        val bitmap = try {
            org.eidora.util.BitmapLoader.loadOrientedBitmap(file, maxSize = 2048)
        } catch (t: Throwable) {
